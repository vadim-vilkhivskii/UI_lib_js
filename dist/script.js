...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.accordion = function () {
  ...
  for (let i = 0; i < this.length; i++) {
    Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).click(() => {
      //* вешам обработчик на все заголовки аккордеона
      Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).toggleClass(headActive); //*при клике тоглим класс активности

      Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i].nextElementSibling).toggleClass(contentActive); //*ищем соседний элемент и тоглим класс активности

      if (this[i].classList.contains(headActive)) {
        //*елси у элемента на который кликнули есть класс активности 
        ...
        //*если класа активности нет тогда устанавливаем высоту следующего элементы в ноль
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.carousel = function () {
  for (let i = 0; i < this.length; i++) {
    ...
    const dots = this[i].querySelectorAll('.carousel-indicators li'); //*находим блок со слайдами и устанавливаем ширину которая будет зависеть от количества слайдов
    ...
    let slideIndex = 0; //* смена слайда

    Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i].querySelector('[data-slide="next"]')).click(e => {
      //*обработчик на срелку переключения слайда
      e.preventDefault();

      if (offset == +width.replace(/\D/g, '') * (slides.length - 1)) {
        //*если дошли до последнего слайда то обнуляем offset
        offset = 0;
      } else {
    ...
    Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i].querySelector('[data-slide="prev"]')).click(e => {
      ...
      if (offset == 0) {
        //*если слайд первый то смещаем на последний
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.tab = function () {
  for (let i = 0; i < this.length; i++) {
    Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).on('click', () => {
      //*вешаем обработчик на каждый tab-item
      Object(_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]) //* обращение к жлементу с которым взаимодействуем 
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.index = function () {
  ...
  const findMyIndex = item => {
    //*Фильтрационная Функция поиска в масиве
    return item == this[0];
  };
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.find = function (selector) {
  ...
  for (let i = 0; i < copyObj.length; i++) {
    ...
    if (arr.length == 0) {
      //*Проверка если элеменов не найдено то пропускаем итерацию цикла
      continue;
    }

    for (let j = 0; j < arr.length; j++) {
      //*запись в главный объект найденных элементов
      this[counter] = arr[j];
      counter++;
    }
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.animateOverTime = function (dur, cb, fin) {
  //* dr - длительность, cd - колбек, финальная функция
  let timeStart; //*Время начала анимации

  function _animateOverTime(time) {
    if (!timeStart) {
      timeStart = time; //*запись стартового времени
    }

    let timeElepsed = time - timeStart; //*Отслеживаем выполнение анимации 

    let complection = Math.min(timeElepsed / dur, 1); //*изменение параметров на странице
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.get = async function (url) {
  let dataTypeAmswer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'json';
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.post = async function (url, data) {
  let dataTypeAmswer = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'text';
  let res = await fetch(url, {
    method: "POST",
    body: data
  });
...
Object(_lib_lib__WEBPACK_IMPORTED_MODULE_0__["default"])('.wrap').html(`  <div class="dropdown">
    <button class="btn btn-primary dropdown-toggle" id="dropdownMenuButton">Dropdaun button</button>
